/**
 * Smoke test: @js-temporal/polyfill + toml-patch (toml-patch API only).
 *
 * Tests only the Temporal APIs that toml-patch actually uses (parse,
 * stringify, patch). Does not probe the full Temporal surface.
 *
 * Usage: node scripts/smoke-temporal-polyfill-toml.mjs
 */
