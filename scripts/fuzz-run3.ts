// Fuzz-runner variant for TOML with the generator's normal compact dotted-key syntax.
//
// Run a single seed:
//   npx -y tsx scripts/fuzz-run3.ts --seed 103 --mutations 3
// Run an inclusive range:
//   npx -y tsx scripts/fuzz-run3.ts --seed 0 --to 100000 --mutations 3
// Stop at the first failure:
//   npx -y tsx scripts/fuzz-run3.ts --seed 0 --to 100000 --mutations 3 --fast-fail
import { fuzzOne3 } from '../src/__tests__/fuzz-patch3';
...
for (let current = seed; current <= to; current++) {
  scanned++;
  const result = fuzzOne3(current, mutations);
  if (result.status !== 'ok') {
