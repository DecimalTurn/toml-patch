 ...
 * stringify, patch). Does not probe the full Temporal surface, so it
 * passes on any runtime where toml-patch's Temporal integration works.
 *
 * Requires --harmony-temporal flag on Node 22-24 (native on Node 26+).
 * Usage: node --harmony-temporal scripts/smoke-temporal-harmony-toml.cjs
 */
...
console.log('[smoke-toml] Temporal available:', typeof Temporal !== 'undefined');
...
console.log('[smoke-toml] step 1: parse date-only with temporal:true');
...
console.log('[smoke-toml] step 2: parse offset datetime → ZonedDateTime');
...
console.log('[smoke-toml] step 3: stringify PlainDate');
check(stringify({ d: obj.d }, FMT) === 'd = 2024-01-15', 'stringify PlainDate');

console.log('[smoke-toml] step 4: stringify ZonedDateTime');
...
console.log('[smoke-toml] step 5: patch no-op');
check(patch('d = 2024-01-15\n', { d: obj.d }, FMT) === 'd = 2024-01-15', 'patch no-op');

console.log('[smoke-toml] step 6: parse without temporal (default Date)');
