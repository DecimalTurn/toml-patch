...
/**
 * The horizontal gap a separating comma occupies between two adjacent inline
 * siblings: 1 column for a compact list (`[1,2]`), 2 for a spaced list
 * (`[1, 2]`).  Detected once at parse time and stored in a WeakMap registry
 * (`inline-comma-space`), so it stays accurate even after later edits shift
 * the items' own positions.  Defaults to 2 (spaced) for containers the parser
 * didn't tag (e.g. values regenerated by parseJS).
 */
function commaSpaceOf(container: InlineArray | InlineTable): number {
  return getCommaSpace(container) ?? 2;
}
...
function insertInline(
  ...
  return calculateInlinePositioning(parent, child, index, {
    ...
    skipCommaSpace: skip_comma_space,
...
export function remove(root: Root, parent: TreeNode, node: TreeNode, hostItems?: TreeNode[]) {
  ...
  // The comma occupies 1 column in a compact list (`[1,2]`) and 2 in a spaced list
  // (`[1, 2]`); read the container's parse-time spacing instead of assuming ", ".
  const removed_comma_gap =
    (isInlineArray(parent) || isInlineTable(parent))
      ? commaSpaceOf(parent as InlineArray | InlineTable)
      : 2;

  if (is_inline && previous_on_same_line) {
    offset.columns -= removed_comma_gap;
  }
