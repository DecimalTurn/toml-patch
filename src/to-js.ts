...
export function toValue(node: Value, integersAsBigInt: IntegersAsBigInt = 'asNeeded'): any {
  switch (node.type) {
    case NodeType.InlineTable:
      ...
      node.items.forEach(({ item }) => {
        ...
        if (defined_keys.has(full_key)) {
          throwError(`Duplicate key "${full_key}" in inline table`);
        }
        ...
        for (let i = 1; i < key.length; i++) {
          ...
          if (defined_keys.has(prefix)) {
            throwError(`Key "${full_key}" conflicts with key: "${prefix}"`);
          }
        }
        ...
        if (defined_prefixes.has(full_key)) {
          ...
          throwError(`Key "${full_key}" conflicts with key: "${existing}"`);
        }
    ...
    default:
      throwError(`Unrecognized value type "${(node as TreeNode).type}"`);
  }
}
...
function validateKey(
  ...
  if (type === NodeType.KeyValue && key.length > 1) {
    for (let i = 1; i < key.length; i++) {
      ...
      if (state.inline_tables.has(candidate)) {
        throwError(`Cannot extend inline table at ${candidate}`);
      }
    }
  }
  ...
  if ((type === NodeType.Table || type === NodeType.TableArray) && state.inline_tables.has(joined_full_key)) {
    throwError(`Cannot extend inline table at ${joined_full_key}`);
  }
  ...
  if (type === NodeType.KeyValue && key.length > 1) {
    for (let i = 1; i < key.length; i++) {
      ...
      if (state.table_arrays.has(candidate)) {
        throwError(`Cannot traverse Array of Tables at ${candidate}`);
      }
    }
  }
  ...
  if ((type === NodeType.Table || type === NodeType.TableArray) && state.implicit_tables.has(joined_full_key)) {
    throwError(`Implicit table already defined: ${joined_full_key}`);
  }
  ...
  if (type === NodeType.KeyValue && state.implicit_tables.has(joined_full_key)) {
    throwError(`Table already defined: ${joined_full_key}`);
  }
  ...
  if (type === NodeType.KeyValue && key.length > 1) {
    for (let i = 1; i <= key.length; i++) {
      ...
      if (state.tables.has(candidate)) {
        throwError(`Cannot extend explicit table ${candidate} with dotted keys`);
      }
    }
  }
  ...
  for (const part of key) {
    ...
    if (isPrimitive(object[part])) {
      throwError(`Value already defined for ${candidates[index]}`);
    }

    const joined_parts = candidates[index];
    if (Array.isArray(object[part]) && !state.table_arrays.has(joined_parts)) {
      throwError(`Cannot add to static array at ${joined_parts}`);
    }
  ...
  if (object && type === NodeType.Table && state.defined.has(joined_key)) {
    throwError(`Table already defined: ${joined_key}`);
  }
  ...
  if (object && type === NodeType.TableArray && !state.table_arrays.has(joined_key)) {
    throwError(`Cannot add Array of Tables to table at ${joined_key}`);
  }
}
