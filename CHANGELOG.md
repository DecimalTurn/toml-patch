...
- Build: add a tree-shaking validation benchmark with size and runtime smoke checks, wired into CI. ([#309])
...
- Stringify: Improve stringification performance when no formatting options are provided ([#313])
...
- Patching: fix `updateOrder` handling for dotted keys and inline-table members while preserving comments ([#292])
