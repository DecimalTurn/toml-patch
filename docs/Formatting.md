...
- **Default:** `false`, auto-detected when patching
- **Description:** Whether generated structural indentation uses tabs instead of spaces. Existing source formatting and multiline string content are preserved.
...
- **Type:** `boolean`
- **Default:** `true`
- **Description:** Whether hex escape sequences generated by the library use uppercase digits, so `\u007F` rather than `\u007f`.

An escape sequence that is already present in the document is reused as written when its value is patched, so this option only decides the case of the escapes the library has to generate. When patching, that case is auto-detected from the first hex escape in the document, and passing the option overrides the detection. `stringify` writes uppercase unless the option says otherwise.

The lite distribution always writes uppercase and has no equivalent option (see the [Patch-lite guide](Patch-Lite.md)).

```js
const format = TomlFormat.default();
format.escapeSequenceUpperCase = true;  // \u007F
format.escapeSequenceUpperCase = false; // \u007f
```
