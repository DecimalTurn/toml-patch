...
const { help, sample, detailed, package: packageIndex, file, versions, baseline, output, self, _: filter } = mri(process.argv.slice(2).filter(a => a !== '--'), {
...
Examples:
  ...
  pnpm run bench:stringify -- --baseline worktrees/latest/dist/toml-patch.js
...
if (allResults.length > 1) {
  printGlobalSummary(allResults, 'Stringify', compareName);
  const mdFile = getOutputFilename() ?? join(__dirname, '..', 'benchmark-stringify.md');
  writeMarkdownSummary(allResults, 'Stringify', mdFile, compareName);
